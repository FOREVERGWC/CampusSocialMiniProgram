    ...
    <el-card>
      <el-table v-loading="loading" :cell-style="{ textAlign: 'center' }" :data="records"
        ...
        <el-table-column label="分数" prop="score">
          <template v-slot="{ row }">
            <el-rate v-model="row.score" :colors="colors" :max="10" :low-threshold="3" :high-threshold="7"
                     allow-half disabled/>
          </template>
        </el-table-column>
    ...
    <el-dialog :title="form.title" v-model="form.visible" destroy-on-close width="40%">
      <el-form ref="formRef" :model="form.data" :rules="rules" label-width="80px">
        ...
        <el-form-item label="分数" prop="score">
          <el-rate v-model="form.data.score" :colors="colors" :max="10" :low-threshold="3" :high-threshold="7"
                   allow-half/>
        </el-form-item>
...
const handleSave = () => {
  formRef.value.validate(valid => {
    if (!valid) return
    saveRateRecord(form.value.data).then(res => {
      if (res.code !== 200) {
