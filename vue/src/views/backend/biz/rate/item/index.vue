		...
		<el-dialog :title="form.title" v-model="form.visible" destroy-on-close width="40%">
			<el-form ref="formRef" :model="form.data" :rules="rules" label-width="80px">
				...
				<el-form-item v-if="form.data.id" label="图片">
					<picture-upload v-model="form.data.attachmentList" :bizId="form.data.id" :bizType="10" />
				</el-form-item>
...
import { getRateItemOne, getRateItemPage, removeRateItemBatchByIds, saveRateItem } from '@/api/biz/rate/item.js'
...
const { loading, records, getRecords, pagination, selectedKeys, single, multiple, handleSelectionChange, onDelete } =
	useTable(page => getRateItemPage({ ...queryParams, pageNo: page.pageNo, pageSize: page.pageSize }), {
		immediate: false
	})
const rateList = ref([])
const form = ref({
...
const handleDelete = id => {
	...
	onDelete(() => removeRateItemBatchByIds(params), {})
}
