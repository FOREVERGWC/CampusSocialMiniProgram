...
import {getPartnerSubjectOne, getPartnerSubjectPage, removePartnerSubjectBatchByIds, savePartnerSubject} from '@/api/partner/subject'
...
const handleSave = () => {
  formRef.value.validate(valid => {
    if (!valid) return
    savePartnerSubject(form.value.data).then(res => {
      if (res.code !== 200) {
