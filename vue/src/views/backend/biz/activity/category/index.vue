...
import {
	getActivityCategoryOne,
	getActivityCategoryPage,
	removeActivityCategoryBatchByIds,
	saveActivityCategory
} from '@/api/biz/activity/category.js'
...
const { loading, records, getRecords, pagination, selectedKeys, single, multiple, handleSelectionChange, onDelete } =
	useTable(page => getActivityCategoryPage({ ...queryParams, pageNo: page.pageNo, pageSize: page.pageSize }), {
		immediate: false
	})
const form = ref({
	visible: false,
...
const showEdit = row => {
	...
	getActivityCategoryOne(params).then(res => {
...
const handleSave = () => {
	formRef.value.validate(valid => {
		if (!valid) return
		saveActivityCategory(form.value.data)
			.then(res => {
				if (res.code !== 200) {
...
const handleDelete = id => {
	...
	onDelete(() => removeActivityCategoryBatchByIds(params), {})
}

const handleExport = () => {
	downloadFile('/activity/category/export', queryParams)
}
