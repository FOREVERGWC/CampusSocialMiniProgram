...
const { loading, records, getRecords, pagination, selectedKeys, single, multiple, handleSelectionChange, onDelete } =
	useTable(page => getNoteCategoryPage({ ...queryParams, pageNo: page.pageNo, pageSize: page.pageSize }), {
		immediate: false
	})
const form = ref({
	visible: false,
...
const handleSave = () => {
	formRef.value.validate(valid => {
		if (!valid) return
		saveNoteCategory(form.value.data)
			.then(res => {
				if (res.code !== 200) {
...
const handleDelete = id => {
	...
	onDelete(() => removeNoteCategoryBatchByIds(params), {})
}
