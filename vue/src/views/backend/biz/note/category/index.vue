<template>
  <div>
    ...
    <el-card>
      <el-table v-loading="loading" :cell-style="{ textAlign: 'center' }" :data="noteCategoryList"
...
const getPage = () => {
  loading.value = true
  getNoteCategoryPage(queryParams).then(res => {
    noteCategoryList.value = res.data?.records || []
...
const showEdit = (row) => {
  ...
  getNoteCategoryOne(params).then(res => {
...
const handleSave = () => {
  formRef.value.validate(valid => {
    if (!valid) return
    saveNoteCategory(form.value.data).then(res => {
      if (res.code !== 200) {
...
const handleDelete = (id) => {
  ...
  removeNoteCategoryBatchByIds(params).then(res => {
    if (res.code !== 200) {
      ElMessage.error(res.msg)
      return
    }
...
const handleExport = () => {
  downloadFile('/note/category/export', queryParams)
}
