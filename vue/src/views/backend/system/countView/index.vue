<template>
  <div>
      ...
      <el-col :span="24">
        <el-card>
          <el-row :gutter="20">
            ...
            <el-col :lg="4" :md="4" :sm="12" :xl="4" :xs="12">
              <el-input v-model="queryParams.count" clearable placeholder="请输入浏览"/>
            </el-col>
    ...
    <el-card>
      <el-table v-loading="loading" :cell-style="{ textAlign: 'center' }" :data="countViewList"
        ...
        <el-table-column label="浏览" prop="count"/>
    ...
    <el-dialog :title="form.title" v-model="form.visible" destroy-on-close width="40%">
      <el-form ref="formRef" :model="form.data" :rules="rules" label-width="80px">
        ...
        <el-form-item label="浏览" prop="count">
...
import {getCountViewOne, getCountViewPage, removeCountViewBatchByIds, saveCountView} from '@/api/countView.js'
...
const rules = {
  ...
  count: [{required: true, message: '请输入浏览', trigger: 'blur'}]
}
...
const getPage = () => {
  ...
  getCountViewPage(queryParams).then(res => {
    countViewList.value = res.data?.records || []
...
const showEdit = (row) => {
  ...
  getCountViewOne(params).then(res => {
...
const handleSave = () => {
  formRef.value.validate(valid => {
    if (!valid) return
    saveCountView(form.value.data).then(res => {
      if (res.code !== 200) {
...
const handleDelete = (id) => {
  ...
  removeCountViewBatchByIds(params).then(res => {
    if (res.code !== 200) {
      ElMessage.error(res.msg)
      return
    }
...
const handleExport = () => {
  downloadFile('/countView', queryParams)
}
