<template>
  <div>
      ...
      <el-col :span="24">
        <el-card>
          <el-row :gutter="20">
            ...
            <el-col :lg="4" :md="4" :sm="12" :xl="4" :xs="12">
              <el-input v-model="queryParams.count" clearable placeholder="请输入评论"/>
            </el-col>
    ...
    <el-card>
      <el-table v-loading="loading" :cell-style="{ textAlign: 'center' }" :data="countCommentList"
        ...
        <el-table-column label="评论" prop="count"/>
    ...
    <el-dialog :title="form.title" v-model="form.visible" destroy-on-close width="40%">
      <el-form ref="formRef" :model="form.data" :rules="rules" label-width="80px">
        ...
        <el-form-item label="评论" prop="count">
...
import {
  getCountCommentOne,
  getCountCommentPage,
  removeCountCommentBatchByIds,
  saveCountComment
} from '@/api/comment/count.js'
...
const rules = {
  ...
  count: [{required: true, message: '请输入评论', trigger: 'blur'}]
}

const getPage = () => {
  loading.value = true
  getCountCommentPage(queryParams).then(res => {
    countCommentList.value = res.data?.records || []
...
const showEdit = (row) => {
  ...
  getCountCommentOne(params).then(res => {
...
const handleSave = () => {
  formRef.value.validate(valid => {
    if (!valid) return
    saveCountComment(form.value.data).then(res => {
      if (res.code !== 200) {
...
const handleDelete = (id) => {
  ...
  removeCountCommentBatchByIds(params).then(res => {
    if (res.code !== 200) {
      ElMessage.error(res.msg)
      return
    }
...
const handleExport = () => {
  downloadFile('/countComment', queryParams)
}
