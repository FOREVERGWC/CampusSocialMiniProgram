<template>
  <div>
      ...
      <el-col :span="24">
        <el-card>
          <el-row :gutter="20">
            ...
            <el-col :lg="4" :md="4" :sm="12" :xl="4" :xs="12">
              <el-input v-model="queryParams.count" clearable placeholder="请输入评分量"/>
            </el-col>
    ...
    <el-card>
      <el-table v-loading="loading" :cell-style="{ textAlign: 'center' }" :data="countRateList"
                :header-cell-style="{ textAlign: 'center' }" stripe
                @selection-change="handleSelectionChange">
        <el-table-column type="selection" width="55"/>
        <el-table-column label="序号" type="index" width="70"/>
        <el-table-column label="评分ID" prop="rateId"/>
        <el-table-column label="评分量" prop="count"/>
    ...
    <el-dialog :title="form.title" v-model="form.visible" destroy-on-close width="40%">
      <el-form ref="formRef" :model="form.data" :rules="rules" label-width="80px">
        ...
        <el-form-item label="评分量" prop="count">
...
import {getCountRateOne, getCountRatePage, removeCountRateBatchByIds, saveCountRate} from '@/api/rate/count.js'
...
const rules = {
  ...
  count: [{required: true, message: '请输入评分量', trigger: 'blur'}]
}
...
const getPage = () => {
  ...
  getCountRatePage(queryParams).then(res => {
    countRateList.value = res.data?.records || []
...
const showEdit = (row) => {
  ...
  getCountRateOne(params).then(res => {
...
const handleSave = () => {
  formRef.value.validate(valid => {
    if (!valid) return
    saveCountRate(form.value.data).then(res => {
      if (res.code !== 200) {
...
const handleDelete = (id) => {
  ...
  removeCountRateBatchByIds(params).then(res => {
    if (res.code !== 200) {
      ElMessage.error(res.msg)
      return
    }
...
const handleExport = () => {
  downloadFile('/countRate', queryParams)
}
