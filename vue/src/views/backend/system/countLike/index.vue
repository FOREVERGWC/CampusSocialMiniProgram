<template>
  <div>
      ...
      <el-col :span="24">
        <el-card>
          <el-row :gutter="20">
            ...
            <el-col :lg="4" :md="4" :sm="12" :xl="4" :xs="12">
              <el-input v-model="queryParams.count" clearable placeholder="请输入赞"/>
            </el-col>
    ...
    <el-card>
      <el-table v-loading="loading" :cell-style="{ textAlign: 'center' }" :data="countLikeList"
        ...
        <el-table-column label="赞" prop="count"/>
    ...
    <el-dialog :title="form.title" v-model="form.visible" destroy-on-close width="40%">
      <el-form ref="formRef" :model="form.data" :rules="rules" label-width="80px">
        ...
        <el-form-item label="赞" prop="count">
...
import {getCountLikeOne, getCountLikePage, removeCountLikeBatchByIds, saveCountLike} from '@/api/countLike.js'
...
const rules = {
  ...
  count: [{required: true, message: '请输入赞', trigger: 'blur'}]
}
...
const getPage = () => {
  ...
  getCountLikePage(queryParams).then(res => {
    countLikeList.value = res.data?.records || []
...
const showEdit = (row) => {
  ...
  getCountLikeOne(params).then(res => {
...
const handleSave = () => {
  formRef.value.validate(valid => {
    if (!valid) return
    saveCountLike(form.value.data).then(res => {
      if (res.code !== 200) {
...
const handleDelete = (id) => {
  ...
  removeCountLikeBatchByIds(params).then(res => {
    if (res.code !== 200) {
      ElMessage.error(res.msg)
      return
    }
...
const handleExport = () => {
  downloadFile('/countLike', queryParams)
}
