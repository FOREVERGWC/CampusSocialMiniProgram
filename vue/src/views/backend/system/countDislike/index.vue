<template>
  <div>
      ...
      <el-col :span="24">
        <el-card>
          <el-row :gutter="20">
            ...
            <el-col :lg="4" :md="4" :sm="12" :xl="4" :xs="12">
              <el-input v-model="queryParams.count" clearable placeholder="请输入踩"/>
            </el-col>
    ...
    <el-card>
      <el-table v-loading="loading" :cell-style="{ textAlign: 'center' }" :data="countDislikeList"
        ...
        <el-table-column label="踩" prop="count"/>
    ...
    <el-dialog :title="form.title" v-model="form.visible" destroy-on-close width="40%">
      <el-form ref="formRef" :model="form.data" :rules="rules" label-width="80px">
        ...
        <el-form-item label="踩" prop="count">
          <el-input v-model="form.data.count" autocomplete="new"/>
        </el-form-item>
...
import {getCountDislikeOne, getCountDislikePage, removeCountDislikeBatchByIds, saveCountDislike} from '@/api/countDislike.js'
import {getBizList} from '@/api/biz'
...
const rules = {
  ...
  count: [{required: true, message: '请输入踩', trigger: 'blur'}]
}
...
const getPage = () => {
  ...
  getCountDislikePage(queryParams).then(res => {
    countDislikeList.value = res.data?.records || []
...
const showEdit = (row) => {
  ...
  getCountDislikeOne(params).then(res => {
...
const handleSave = () => {
  formRef.value.validate(valid => {
    if (!valid) return
    saveCountDislike(form.value.data).then(res => {
      if (res.code !== 200) {
...
const handleDelete = (id) => {
  ...
  removeCountDislikeBatchByIds(params).then(res => {
    if (res.code !== 200) {
      ElMessage.error(res.msg)
      return
    }
...
const handleExport = () => {
  downloadFile('/countDislike', queryParams)
}
