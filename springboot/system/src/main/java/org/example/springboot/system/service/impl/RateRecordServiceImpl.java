...
import org.example.springboot.system.mapper.RateRecordMapper;
 ...
 */
@Service
public class RateRecordServiceImpl extends ServiceImpl<RateRecordMapper, RateRecord> implements IRateRecordService {
    @Resource
    ...
    public List<RateRecordVo> getList(RateRecordDto dto) {
        List<RateRecord> rateRecordList = getWrapper(dto).list();
        if (CollectionUtil.isEmpty(rateRecordList)) {
            return List.of();
        }
        // 评分
        List<Long> rateIdList = rateRecordList.stream().map(RateRecord::getRateId).toList();
        ...
        // 评分项
        List<Long> rateItemIdList = rateRecordList.stream().map(RateRecord::getRateItemId).toList();
        ...
        // 用户
        List<Long> userIdList = rateRecordList.stream().map(RateRecord::getUserId).toList();
        List<User> userList = userService.listByIds(userIdList);
        Map<Long, User> userMap = userList.stream().collect(Collectors.toMap(User::getId, item -> item));
        // 组装VO
        return rateRecordList.stream().map(item -> {
     ...
     * @return 结果
     */
    private LambdaQueryChainWrapper<RateRecord> getWrapper(RateRecord entity) {
