...
import org.example.springboot.system.mapper.RateMapper;
...
public class RateServiceImpl extends ServiceImpl<RateMapper, Rate> implements IRateService {
    ...
    public List<RateVo> getList(RateDto dto) {
        List<Rate> rateList = getWrapper(dto).list();
        if (CollectionUtil.isEmpty(rateList)) {
            return List.of();
        }
        // 用户
        List<Long> userIdList = rateList.stream().map(Rate::getUserId).toList();
        List<User> userList = userService.listByIds(userIdList);
        Map<Long, User> userMap = userList.stream().collect(Collectors.toMap(User::getId, item -> item));
        // 组装VO
        return rateList.stream().map(item -> {
     ...
     * @return 结果
     */
    private LambdaQueryChainWrapper<Rate> getWrapper(Rate entity) {
